...
export default function AdminDashboard() {
  ...
  try {
    ...
    setContractPaused(paused);           // ← endi to'g'ri
    setEmergencyStatus(emergencyStatusData); // ← endi to'g'ri
    setContractStats(stats);             // ← endi to'g'ri
  } catch (error) {
  ...
  toast.error(
    `${tokenNames[tokenType] ?? "Token"} listing fee is not supported in this contract version.`
  );
};
