...
export default function MyListings() {
  ...
  const handleCancelListing = async (tokenId) => {
    if (!walletClient) { toast.error("Please connect your wallet"); return; }
    try {
      setCancelling(tokenId);
      toast.loading("Cancelling listing...", { id: "cancel-tx" });
      const tx = await cancelListing(walletClient, tokenId);
      await tx.wait();
      toast.success("Listing cancelled — NFT returned to your wallet", { id: "cancel-tx" });
      await new Promise((resolve) => setTimeout(resolve, 2000));
      await loadMyListings();
    } catch (error) {
      const isRejected = error.message?.toLowerCase().includes("user rejected");
      toast.error(isRejected ? "Transaction rejected" : "Failed to cancel listing", { id: "cancel-tx" });
    } finally {
      setCancelling(null);
    }
  };
            ...
            >
              ...
              {!item.sold && (
                <motion.button
                  ...
                  disabled={cancelling === item.tokenId}
                  className="mt-2 w-full flex items-center justify-center py-2.5 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border border-red-200/50 dark:border-red-700/30 hover:bg-red-100 dark:hover:bg-red-900/40 transition-all duration-200 font-medium text-sm disabled:opacity-50"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  {cancelling === item.tokenId ? "Cancelling..." : "✕ Cancel Listing"}
                </motion.button>
              )}
