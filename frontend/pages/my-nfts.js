...
export default function MyNFTs() {
  ...
  const handleCancelListing = async (nft) => {
    ...
    try {
      ...
      const tx = await cancelListing(walletClient, nft.tokenId);
      await tx.wait();
      toast.success("Listing cancelled — NFT returned to your wallet", { id: "cancel-listing" });
      await new Promise((resolve) => setTimeout(resolve, 2000));
      await loadMyNFTs();
    } catch (error) {
      if (error.message?.includes("user rejected")) {
        toast.error("Transaction rejected", { id: "cancel-listing" });
      } else {
        toast.error("Failed to cancel listing", { id: "cancel-listing" });
      }
    } finally {
      setReselling(false);
    }
          ...
          {filteredNFTs.map((nft, index) => {
            ...
            return (
              ...
              >
                ...
                />
                {/* Cancel Listing button — only shown for active listings */}
                {isListed && (
                  <motion.button
                    onClick={() => handleCancelListing(nft)}
                    disabled={reselling}
                    className="mt-2 w-full flex items-center justify-center space-x-2 py-2.5 px-4 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border border-red-200/50 dark:border-red-700/30 hover:bg-red-100 dark:hover:bg-red-900/40 transition-all duration-200 font-medium text-sm disabled:opacity-50"
